...
$section_classes = ['trust-bar-block', 'scroll-reveal'];
if ($heading !== '') {
    $section_classes[] = 'trust-bar-block--has-heading';
}
?>
<section class="<?php echo esc_attr(implode(' ', $section_classes)); ?>" aria-label="<?php esc_attr_e('Key facts', 'atomic-design'); ?>">
    <div class="container trust-bar-block__inner">
        <?php if ($heading !== '') : ?>
            <div class="trust-bar-block__heading-wrap">
                <h2 class="trust-bar-block__heading"><?php echo esc_html($heading); ?></h2>
            </div>
        <?php endif; ?>

        <div class="trust-bar-block__content">
