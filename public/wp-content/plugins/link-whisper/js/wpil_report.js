...
(function ($) {
    ...
    function wpil_report_filter() {
        ...
        var parentContainer = block.parents('.wpil-hamburger-filter-fields');
        if(parentContainer.is(':visible')){
            parentContainer.css('max-height', block.prop('scrollHeight') + 'px');
        }
    }
    wpil_report_filter();
    ...
    function wpil_report_filter_submit() {
        ...
        var linkDensity = (-1 !== window.location.href.indexOf('link_density')) ? '&link_density=1': '';
        var linkRelation = (-1 !== window.location.href.indexOf('link_relation')) ? '&link_relation=1': '';
        var url = wpil_admin_url + 'admin.php?page=link_whisper&type=links' + orphaned + linkDensity + linkRelation + '&filter_type=' + filterType;
