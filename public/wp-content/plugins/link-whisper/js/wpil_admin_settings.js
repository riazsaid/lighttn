...
(function ($)
{
    ...
    function toggleAiSettings(){
        if($('[name="wpil_open_ai_api_key"]').length < 1 || $('[name="wpil_open_ai_api_key"]').val().length < 1){
            return;
        }
        ...
        if(hasChangedSettings){
            $('.wpil-live-download-ai-data.button-primary, .wpil-live-download-ai-data-help-tooltip').css({'display': 'none'});
            $('.wpil-live-download-ai-data-disabled').css({'display': 'inline-block'});
        }else{
            $('.wpil-live-download-ai-data.button-primary, .wpil-live-download-ai-data-help-tooltip').css({'display': 'inline-block'});
    ...
    function toggleShowTopAISuggestions(){
        ...
        var usingAI = $(this).is(':checked');

        if(usingAI){
            $('.wpil-show-top-ai-suggestions, .wpil-disable-ai-anchor-building, .wpil-disable-ai-suggestion-cron').removeClass('hide-setting');
        }else{
    ...
    function liveDownloadAIData(e){
        ...
        if(!$('#wpil-disconnect-ai-subscription').is(":visible")){
            ...
            if($(button).data('wpil-ai-batch-processing-active') != '1'){
                message += '<br><br><div style="font-size="8px"><input type="checkbox" id="wpil-turn-on-batch-ai-processing-popup-button" value="1" checked> Enable AI updater to keep data fresh.</div>';
            }
            $(wrapper).append(message);
            ...
            wpil_swal({
                ...
                if($('#wpil-turn-on-batch-ai-processing-popup-button').is(':checked')){
                    $(button).addClass('wpil-turn-on-batch-ai-processing');
                }
                if (begin) {
                    wpil_swal({
    ...
    function ajaxliveDownloadAIData(button, time = 0, lastPassUnchanged = false){
        ...
        jQuery.ajax({
            ...
            data: {
                ...
                start_time: time,
                activate_batch_processing: ($(button).hasClass('wpil-turn-on-batch-ai-processing') ? 1: 0),
