...
//            if(!isset($_GET['post_type']) || $_GET['post_type'] !== 'url'){
//                $options['link_created'] = __('Link Created', 'wpil');
//            }
        }

        $options['wpil-report-actions'] = __('Actions', 'wpil');
    ...
    function column_default($item, $column_name)
    {
        switch($column_name) {
            ...
            case 'link_created':
                return !empty($item->link_created) ? date(get_option('date_format', 'F d, Y'), $item->link_created): __('Unknown', 'wpil');
            default:
    ...
    function build_action_panel($item){
        ...
        if(is_numeric($_GET['post_id'])){
            $actions = [
                'view-url-clicks' => '<a href="' . esc_url(admin_url("admin.php?post_id=" . urlencode($item->link_url)) . "&post_type=url&page=link_whisper&type=click_details_page&ret_url=" . base64_encode($_SERVER['REQUEST_URI'] . '&direct_return=1')) . '" class="wpil-action-panel-button">' . esc_html__('View Clicks for URL', 'wpil') . '</a>',
                'delete-clicks' => '<a data-click_id="'. ((isset($item->click_id)) ? $item->click_id: 0) .'" data-post_id="'.$item->post_id.'" data-post_type="'.$item->post_type.'" data-anchor="'. esc_attr(((isset($item->link_anchor)) ? $item->link_anchor: '')) .'" data-url="'.base64_encode($item->link_url).'" data-nonce="'. wp_create_nonce(wp_get_current_user()->ID . 'delete_click_data') .'" class="wpil_delete_click_data wpil-action-panel-button">Delete Clicks</a>',
            ];
        }else{
            $post = new Wpil_Model_Post($item->post_id, $item->post_type);
            $actions = [
                'view-post-clicks' =>  '<a href="' . esc_url(admin_url("admin.php?post_id={$post->id}&post_type={$post->type}&page=link_whisper&type=click_details_page&ret_url=" . base64_encode($_SERVER['REQUEST_URI'] . '&direct_return=1'))) . '" class="wpil-action-panel-button">' . esc_html__('View Clicks for Post', 'wpil') . '</a>',
                'delete-clicks' => '<a data-click_id="'. ((isset($item->click_id)) ? $item->click_id: 0) .'" data-post_id="'.$item->post_id.'" data-post_type="'.$item->post_type.'" data-anchor="'. esc_attr(((isset($item->link_anchor)) ? $item->link_anchor: '')) .'" data-url="'.base64_encode($item->link_url).'" data-nonce="'. wp_create_nonce(wp_get_current_user()->ID . 'delete_click_data') .'" class="wpil_delete_click_data wpil-action-panel-button">Delete Clicks</a>',
            ];
        }
    ...
    public function search_box( $text, $input_id ) {
        ...
        <div class="erase-user-ip-data-wrapper">
            <div class="erase-user-ip-data-container">
                <button type="button" class="button-primary erase-user-ip-data" data-user-ip="<?php echo esc_attr($ip_address); ?>" data-nonce="<?php echo wp_create_nonce(wp_get_current_user()->ID . 'delete_click_ip_data'); ?>"><?php esc_html_e('Erase IP Data', 'wpil'); ?></button>
                ...
                <div class="wpil_help" style="float:right;">
                    ...
                    <div style="margin: 30px 0 0 -190px;">
                        <?php esc_html_e("Clicking this button will tell Link Whisper to erase the IP address data associated with these clicks", 'wpil'); ?>
                        <br>
                        <br>
                        <?php esc_html_e("This will not delete the clicks, but will make the clicks anonymous.", 'wpil'); ?>
                    </div>
                </div>
            </div>
        </div>
