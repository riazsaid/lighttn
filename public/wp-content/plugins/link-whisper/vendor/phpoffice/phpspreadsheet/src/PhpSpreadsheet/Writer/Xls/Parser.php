...
class Parser
{
    ...
    {
        $ext_ref = (string) \preg_replace(["/^'/", "/'\$/"], ['', ''], $ext_ref);
    ...
    private function fact()
    {
        ...
        if (\preg_match('/^(\\$)?[A-Ia-i]?[A-Za-z](\\$)?\\d+:(\\$)?[A-Ia-i]?[A-Za-z](\\$)?\\d+$/', $this->currentToken) || \preg_match('/^(\\$)?[A-Ia-i]?[A-Za-z](\\$)?\\d+\\.\\.(\\$)?[A-Ia-i]?[A-Za-z](\\$)?\\d+$/', $this->currentToken)) {
