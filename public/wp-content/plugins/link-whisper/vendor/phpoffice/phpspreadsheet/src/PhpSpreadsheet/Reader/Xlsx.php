...
use LWVendor\PhpOffice\PhpSpreadsheet\DefinedName;
...
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Namespaces;
...
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SharedFormula;
...
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\TableReader;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Theme;
use LWVendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\WorkbookView;
...
use LWVendor\PhpOffice\PhpSpreadsheet\Style\Font as StyleFont;
...
class Xlsx extends BaseReader
{
    ...
    private function loadZip(string $filename, string $ns = '', bool $replaceUnclosedBr = \false) : SimpleXMLElement
    {
        ...
        $rels = @\simplexml_load_string($this->getSecurityScannerOrThrow()->scan($contents), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions(), $ns);
    ...
    {
        $contents = $this->getFromZipArchive($this->zip, $filename);
        $rels = \simplexml_load_string($this->getSecurityScannerOrThrow()->scan($contents), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions(), $ns === '' ? $ns : '');
        return self::testSimpleXml($rels);
    }
            ...
            if ($mainNS !== '') {
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $xml = new XMLReader();
                        $xml->xml($this->getSecurityScannerOrThrow()->scan($this->getFromZipArchive($this->zip, $fileWorksheetPath)), null, Settings::getLibXmlLoaderOptions());
                ...
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        ...
                        if (isset($run->rPr->u)) {
                            ...
                            if (!isset($attr['val'])) {
                                $objFont->setUnderline(\LWVendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE);
                            } else {
    ...
    private function readRibbon(Spreadsheet $excel, string $customUITarget, ZipArchive $zip) : void
    {
        ...
        if ($dataRels) {
            ...
            $UIRels = \simplexml_load_string($this->getSecurityScannerOrThrow()->scan($dataRels), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
            if (\false !== $UIRels) {
     ...
     */
    private function readHyperLinkDrawing(\LWVendor\PhpOffice\PhpSpreadsheet\Worksheet\Drawing $objDrawing, SimpleXMLElement $cellAnchor, $hyperlinks) : void
    {
