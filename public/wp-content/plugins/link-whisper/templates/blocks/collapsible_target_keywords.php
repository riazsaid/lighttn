<div class="wpil-collapsible-wrapper <?php echo ($col_number !== false) ? 'wpil-keyword-col-' . $col_number: '';?>">
    <div class="wpil-collapsible wpil-collapsible-static wpil-links-count <?php echo (!empty($keywords)) ? 'wpil-collapsible-has-data': 'wpil-collapsible-no-data';?>"><?=count($keywords)?></div>
    ...
    <div class="update-post-keywords">
        <a href="#" class="button-primary wpil-update-selected-keywords" data-nonce="<?php echo wp_create_nonce(get_current_user_id() . 'update-selected-keywords-' . $post->id); ?>" data-post-id="<?php echo $post->id; ?>"><?php esc_html_e('Update Active', 'wpil'); ?></a>
    </div>
    <?php } ?>
    <?php if('custom-keyword' === $keyword_type){ ?>
    <div class="create-post-keywords">
        <a href="#" style="vertical-align: top;" class="button-primary wpil-create-target-keywords" data-nonce="<?php echo wp_create_nonce(get_current_user_id() . 'create-target-keywords-' . $post->id); ?>" data-post-id="<?php echo $post->id; ?>" data-post-type="<?php echo $post->type; ?>"><?php esc_html_e('Create', 'wpil'); ?></a>
        <div class="wpil-create-target-keywords-row-container"  style="display: inline-block; width: calc(100% - 200px);">
            <input type="text" style="width: 100%" class="create-custom-target-keyword-input" placeholder="<?php esc_attr_e('New Custom Keyword', 'wpil'); ?>">
        </div>
        <a href="#" class="button-primary wpil-add-target-keyword-row" style="margin-left:0px; vertical-align: top;"><?php esc_html_e('Add Row', 'wpil'); ?></a>
    </div>
    <?php } ?>
</div>