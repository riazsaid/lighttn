...
namespace Soflyy\WpAllImportRapidAddon;

if (!class_exists('Soflyy\WpAllImportRapidAddon\RapidAddon')) {
	...
	class RapidAddon {
		...
		public $repeatable_groups = array();
		...
		function add_field($field_slug, $field_name, $field_type, $enum_values = null, $tooltip = "", $is_html = true, $default_text = '') {
			...
			if ( ! empty($enum_values) ){
				foreach ($enum_values as $key => $value) {
					if (is_array($value))
					{
						if ($field['type'] == 'accordion' || 'repeatable_group' == $field['type'])
						{
    ...
    <h3 class="hndle" style="margin-top:0;"><span>'.$acf_group['title'].'</span></h3>
		...
		function render_field($field_params, $field_slug, $current_values, $in_the_bottom = false){
			...
			} else if ($field_params['type'] == 'select') {
			...
			} else if( 'repeatable_group' == $field_params['type']){
		...
		function get_sub_fields($field_params, $field_slug, $current_values){
			...
			if ( ! empty($field_params['enum_values']) ){
				foreach ($field_params['enum_values'] as $key => $value) {
					...
					if (is_array($value)){
						if ($field_params['type'] == 'accordion' || 'repeatable_group' == $field_params['type']){
		...
		function add_repeatable_group( $main_field = false, $title = '', $fields = [] ) {
			...
			if ( ! empty( $fields ) ) {
				...
				return $this->add_field( 'repeatable_group_' . $fields[0]['slug'], $title, 'repeatable_group', $fields );
			}
		}
