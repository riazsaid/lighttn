...
class Admin_Feeds {
	...
	public function __construct( $settings ) {
		...
		add_action( 'wp_dashboard_setup', array( $this, 'register_rss_feeds' ) );
	}
	...
	public function register_rss_feeds() {
		wp_add_dashboard_widget(
			...
			array( $this, 'display_rss_feeds_content' ),
			null,
			null,
			'column4',
			'core'
	...
	public function display_rss_feeds_content() {
		...
		if ( is_array( $feeds ) ) {
			...
			<a href="<?php echo esc_url( $feed->demo_link ); ?>" target="_blank">
				<?php esc_html_e( 'Check Details', 'semantic-linkboss' ); ?>
			</a>
					</p>
				</div>
				<?php
	...
	private function get_rss_posts_data() {
		...
		if ( ! empty( $cached_data ) ) {
			...
			if ( is_wp_error( $rss ) ) {
				return '<li>' . esc_html__( 'Items Not Found', 'semantic-linkboss' ) . '.</li>';
			}
			...
			$simplified_rss_items = array_map(
				function ( $item ) {
					return array(
		...
		<div class="rss-widget">
			<ul>
				<?php if ( empty( $rss_items ) ) : ?>
					<li><?php esc_html_e( 'Items Not Found', 'semantic-linkboss' ); ?>.</li>
				<?php else : ?>
					<?php foreach ( $rss_items as $item ) : ?>
						<li>
							...
							<span class="rss-date" style="display: block; margin: 0;">
								<?php echo esc_html( human_time_diff( $item['date'], time() ) . ' ' . __( 'ago', 'semantic-linkboss' ) ); ?>
							</span>
...
$settings = array(
	'feed_title'       => 'LinkBoss – Semantic Internal Linking',
	'transient_key'    => 'linkboss_feeds',
	'feed_link'        => 'https://linkboss.io/feed',
	'remote_feed_link' => 'https://plugin.linkboss.io/wp-json/linkboss/v1/products-feed/all?category=linkboss',
	'text_domain'      => 'semantic-linkboss',
	'footer_links'     => array(
		array(
			'url'   => 'https://linkboss.io/blog/',
			'title' => 'Blog',
		),
		array(
			'url'   => 'https://linkboss.io/knowledgebase/',
			'title' => 'Docs',
		),
		array(
			'url'   => 'https://linkboss.io/#pricing',
			'title' => 'Get Pro',
		),
		array(
			'url'   => 'https://linkboss.io/changelog/',
			'title' => 'Changelog',
		),
	),
);
